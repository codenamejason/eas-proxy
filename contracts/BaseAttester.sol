...
/// @title BaseAttester
/// @dev A contract that allows a Verifier contract to add review information for users using Ethereum Attestation Service.
contract BaseAttester is Ownable {
    ...
    // Note: who makes/decides this list?
    ...
    /// @dev Adds review information for a user using EAS
    /// @param multiAttestationRequest An array of `MultiAttestationRequest` structures containing the user's review information.
    ...
    function revokeAttestations(MultiRevocationRequest[] calldata multiRevocationRequest) public payable virtual {
        require(
            verifiers[msg.sender] || msg.sender == owner(), "Only authorized verifiers or owner can call this function"
        );
