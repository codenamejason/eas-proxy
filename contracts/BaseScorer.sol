...
/// @title BaseScorer
/// @dev A contract that scores a user based on his EAS review attestations
contract BaseScorer is Ownable {
    // The instance of the EAS contract.
    IEAS eas;


    /// @dev Store the weights for each stamp. This should be used for scoring.
    /// This is a nested array. Firs index is the index of the provider in the providers array of the review attestation.
    ...
    function scorePublication(address recipient) public payable virtual returns (uint256 score) {
        score = 0;
        ...
        (providers, hashes) = abi.decode(attestation.data, (bytes32[], bytes32[]));
    }
}
