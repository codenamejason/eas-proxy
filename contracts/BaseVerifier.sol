 ...
 */
contract BaseVerifier is Ownable {
    using ECDSA for bytes32;

    // Instance of the GitcoinAttester contract
    BaseAttester public immutable attester;
    ...
    bytes32 private constant MULTI_ATTESTATION_REQUEST_TYPEHASH = keccak256(
    ...
    constructor(address _issuer, address _attester) {
        attester = BaseAttester(_attester);
        issuer = _issuer;
        name = "BaseVerifier";
    ...
    function hashMultiAttestationRequest(MultiAttestationRequest memory _request) private pure returns (bytes32) {
        ...
        for (uint256 i = 0; i < _request.data.length;) {
        ...
        return keccak256(
            abi.encode(MULTI_ATTESTATION_REQUEST_TYPEHASH, _request.schema, keccak256(abi.encodePacked(dataHashes)))
        );
    }
    ...
    function _hashAttestations(PassportAttestationRequest calldata attestationRequest) private pure returns (bytes32) {
        ...
        for (uint256 i = 0; i < attestationRequest.multiAttestationRequest.length;) {
     ...
     */
    function _verify(uint8 v, bytes32 r, bytes32 s, PassportAttestationRequest calldata attestationRequest) internal {
     ...
     * @param s The s component of the signature.
     */
    function verifyAndAttest(PassportAttestationRequest calldata attestationRequest, uint8 v, bytes32 r, bytes32 s)
        public
        payable
    {
